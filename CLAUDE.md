...
32. **Notion fetch scripts authenticate `notion-client` with `NOTION_TOKEN_V2`/`NOTION_ACTIVE_USER` (session cookie), not the official API.** Notion's private `www.notion.so/api/v3/*` endpoints started 403ing unauthenticated GitHub Actions IPs in Aug 2026, even for public pages. The official `@notionhq/client` API would be the durable fix but requires creating a workspace integration, which is blocked while the account is a Guest (not a Member) in that workspace. `createNotionApi()` in `scripts/notion-utils.mjs` passes a logged-in member's `token_v2`/`notion_user_id` cookies instead. This is an interim unblock: the cookie can expire or be invalidated with no warning, at which point re-extract it from a browser session and re-set the two repo secrets. Revisit the official-API migration once integration-creation permission exists.
